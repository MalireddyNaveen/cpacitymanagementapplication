...
sap.ui.define([
    ...
    function (BaseController, MessageBox, MessageToast, JSONModel, fioriLibrary, Filter, FilterOperator) {
      "use strict";
  
      return BaseController.extend("com.app.capacitymanagement.controller.SimulationDetails", {
        onInit() {
          ...
          oRouter.attachRoutePatternMatched(this.onSimulationDetailsLoad_CM, this);
  
          //Background for the Icons are Tarnsparent..
          var icontoolheaders_SimulatedPage = [
            this.getView().byId("container-com.sap.capacitymanagementapp---SimulationDetails--idIconTabBar--header"),
          ];
        ...
        onSimulationDetailsLoad_CM: async function (oEvent) {
          ...
          var oFilter = new Filter("simulationName_simulationName", FilterOperator.EQ, simID);
        ...
        onTabSelect: function (oEvent) {
          ...
          this.getView().getModel("detailView").setProperty("/selectedTab", selectedKey);  // Update the selected tab in the model
        },
        ...
        onDataVisualization() {
          ...
          const oContainerObject = oContainerTable.getItems()[0].getBindingContext().getObject("truckType");
  ...
  let t = oRowContext.getObject().SelectedQuantity;
            ...
            let newGrossWeight = Number(grossWeight) * Number(oSelectedQuantity);
        ...
        onPressBtnPDFDownloadPreviousModelsData: async function () {
          ...
          const oSingleSimulationData = oSimulationData.results.find((Simulation) => Simulation.simulationName === oSimulationID);
        ...
        onAutoBindingTheTableData: function (oTable) {
          var columns = ["Product ID", "Product Description", "Quantity(Pcs)", "Volume(m³)"];
          var rows = [];
